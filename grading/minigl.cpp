...
void Rasterize_Triangle(const triangle &t, MGLsize w, MGLsize h, MGLpixel *data)
{
    ...
    int minx = max(min(a[0],min(b[0],c[0])),0.0f);
    int miny = max(min(a[1],min(b[1],c[1])),0.0f);
    int maxx = min(max(a[0],max(b[0],c[0])),w-1.0f);
    int maxy = min(max(a[1],max(b[1],c[1])),h-1.0f);
